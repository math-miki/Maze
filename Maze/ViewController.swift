...
class ViewController: UIViewController {
    ...
    func startAccelerometer() {
        let handler: CMAccelerometerHandler = {(accelerometerData: CMAccelerometerData?, error: Error?) -> Void in
            if error != nil {
                print("error:", error)
                return
            }
            guard let data = accelerometerData else {
                print("cannot treat data")
                return
            }
    ...
    func gameCheck(result: String, message: String) {
        ...
        let gameCheckAction =  UIAlertAction(title: "もう一度", style: .default) { (act) in
            self.retry()
        }
        
        gameCheckAlert.addAction(gameCheckAction)
